...
/datum/mind/proc/edit_memory()
	...
	if(ishuman(current))
		...
		/** The borg **/ //SKREE
		text = "borg"
	//	if (SSticker.mode.config_tag=="borg")
	//		text = uppertext(text)
		text = "<i><b>[text]</b></i>: "
		if (src in SSticker.mode.hivemind.borgs)
			...
			text += "<a href='?src=\ref[src];xel=makexel'>borg drone</a>|<b>employee</b>"
...
/datum/mind/Topic(href, href_list)
	...
	else if(href_list["borg"]) //SKREE
		switch(href_list["borg"])
			if("clear")
				if(src in SSticker.mode.hivemind.borgs)
					SSticker.mode.borgs -= src
					to_chat(src, "<span class='userdanger'>The voices in our head go silent, we are not longer a Xel!</span>")
					remove_xel(src)
			...
			if("makexel")
				if(!ishuman(current))
					...
					return
				make_xel(src)
				to_chat(current, "<span class='userdanger'>You start to hear weird voices as a new organ shifts in your chest. You're a xel drone now!</span>")
				message_admins("[key_name_admin(usr)] has Xel'ed [current].")
				log_admin("[key_name(usr)] has borg drone'ed [current].")
			if("toship")
				message_admins("[key_name_admin(usr)] has sent [current] to the borg mothership.")
				log_admin("[key_name(usr)] has sent [current] to the borg mothership.")
